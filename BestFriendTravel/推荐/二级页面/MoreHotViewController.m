...
@interface MoreHotViewController ()<UITableViewDataSource,UITableViewDelegate>
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title=@"热门游记";
    [self customNav];
...
-(void)loadData{
    [self.hud show:YES];
    [CommonModel requestDataWithSn:_Sn withURL:HOT_URL complectionBlock:^(NSMutableArray *array, NSError *error) {
        if (error==nil) {
            if (_Sn==0) {
